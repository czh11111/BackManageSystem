...
export default {
  ...
  data() {
    return {
      tableData: [],//表单里的所有数据，刚创建、数据修改、删除、导入数据后从后端获取
      total: 0,
      pageNum: 1,
      pageSize: 10,
      name: "",//记录搜索框内容，点重置变为空字符串
      form: {},//存储表单里被修改的那条数据，修改结束会被放回后端
      dialogFormVisible: false,//用于控制表单编辑弹窗是否显示
      multipleSelection: [],//记录哪些数据被选中
      options: []//菜单可以选择的图标样式，从后端获取
    }
  },
  created() {
    this.load()
